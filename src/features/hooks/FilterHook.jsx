...
const useFilter = (initialFilters=[]) => {
    ...
    useEffect(() => {
        // Only set default secondary option if primarySortOption is not empty
        // This prevents setting a default when we're resetting sorting
        if (primarySortOption) {
            // Set a default secondary option based on the primary option
            // For Score, Name, and Activity, default to descending
            if (['Score', 'Name', 'Activity'].includes(primarySortOption)) {
                const defaultSecondaryOption = 'descending';
