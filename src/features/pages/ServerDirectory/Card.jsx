...
const Card = ({serverData, ratingThreshold, activityLevelStyles, locationStyles, featureTagStyles, openModalOnClick}) => {
    return (
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 flex flex-col hover:shadow-lg transition-shadow duration-200 cursor-pointer border border-gray-200 dark:border-gray-700 overflow-hidden" onClick={() => openModalOnClick(serverData)}>
            <div className="flex flex-col">
                <div>
                    <div className="flex justify-between items-start">
                        <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 pr-2">{serverData.name}</h3>
                        <button className="text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors flex-shrink-0">
                    ...
                    <div className="flex items-center flex-wrap gap-x-3 gap-y-2 text-sm text-gray-500 dark:text-gray-400 mt-1">
                        ...
                        <span className="font-semibold text-gray-800 dark:text-gray-200 text-base">{serverData.rating}</span>
                        <div className="flex items-center gap-x-3 flex-shrink-0">
                            <span className="px-2 py-1 border border-gray-400 dark:border-gray-500 rounded-full text-xs font-semibold text-gray-700 dark:text-gray-300 whitespace-nowrap">{serverData.tag}</span>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-4 text-sm line-clamp-4">{serverData.description}</p>
                </div>
                <div className="mt-5">
